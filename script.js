...
const aboutBentoInner = document.querySelector('.about-bento-inner');
...
const worksRunway = document.querySelector('[data-works-runway]');
const worksStage = document.querySelector('[data-works-stage]');
const worksViewport = document.querySelector('[data-works-viewport]');
const worksTrack = document.querySelector('[data-works-track]');
const worksProjectOrder = [
  'ai-thrombolysis',
  'adhd-ai',
  'search-focus',
  'mark-code',
...
if (worksTrack) {
  worksProjectOrder.forEach((projectName) => {
    const projectCard = worksTrack.querySelector(`[data-project-gallery="${projectName}"]`);
    if (projectCard) {
      worksTrack.append(projectCard);
    }
  });
}
...
const worksCategoryTitle = document.querySelector('[data-works-category-title]');
...
const updateSiteScale = () => {
  ...
  const chatShiftFactor = viewportWidth >= 1800 ? 0.48 : viewportWidth >= 1440 ? 0.42 : viewportWidth > 1100 ? 0.32 : 0;
  const chatPanelWidth = viewportWidth >= 1440 ? 560 : Math.min(560, viewportWidth * 0.42);
  const bentoChatCompensation = chatShiftFactor ? (chatPanelWidth * chatShiftFactor) / scale : 0;

  document.documentElement.style.setProperty('--site-scale', scale.toFixed(4));
  document.documentElement.style.setProperty('--bento-chat-compensation', `${bentoChatCompensation.toFixed(2)}px`);
...
const siteTranslations = {
  zh: {
    ...
    heroEyebrow: 'Portfolio 2026 · Product / UX Designer',
    heroStatement: 'Designing AI<br />experiences that<br />empower people.',
    heroCredit: 'Long Xiangyu · Product / UX Designer',
    heroContact: '联系',
    copyEmail: '复制邮箱',
    aboutTitleCn: '关于我',
    aboutTitleEn: 'About',
...
const projectLanguageMap = {
  'ai-thrombolysis': {
    zh: { title: 'AI 溶栓助手', tags: ['AI Product'] },
    en: { title: 'AI Thrombolysis Assistant', tags: ['AI Product'] },
  },
  'search-focus': {
    zh: { title: 'Search Focus', tags: ['AI Workflow'] },
    en: { title: 'Search Focus', tags: ['AI Workflow'] },
  },
  'mark-code': {
    zh: { title: 'MarkCode', tags: ['AI Workflow'] },
    en: { title: 'MarkCode', tags: ['AI Workflow'] },
  },
  relight: {
    zh: { title: 'Relight', tags: ['Others'] },
    en: { title: 'Relight', tags: ['Others'] },
  },
  iknow: {
    zh: { title: 'iKnow', tags: ['Others'] },
    en: { title: 'iKnow', tags: ['Others'] },
  },
  'adhd-ai': {
    zh: { title: 'AI如何帮助 ADHD？', tags: ['AI Product'] },
    en: { title: 'How AI Helps ADHD?', tags: ['AI Product'] },
  },
  etea: {
    zh: { title: 'E-TEA', tags: ['Others'] },
    en: { title: 'E-TEA', tags: ['Others'] },
  },
  'marry-christmas': {
    zh: { title: 'Merry Christmas', tags: ['Others'] },
    en: { title: 'Merry Christmas', tags: ['Others'] },
  },
};
...
if (worksRunway && worksStage && worksViewport && worksTrack && worksCategoryTitle && workCards.length) {
  ...
  const setActiveCategory = (category) => {
    ...
    const nextTitle = categoryTitles[category] || category;
    const isInitial = !activeCategory;
    activeCategory = category;
    window.clearTimeout(categoryTitleTimer);

    if (isInitial || reducedMotionMedia.matches) {
      worksCategoryTitle.textContent = nextTitle;
      worksCategoryTitle.classList.remove('is-leaving', 'is-entering');
      return;
    }

    worksCategoryTitle.classList.remove('is-entering');
    worksCategoryTitle.classList.add('is-leaving');
    categoryTitleTimer = window.setTimeout(() => {
      worksCategoryTitle.textContent = nextTitle;
      worksCategoryTitle.classList.remove('is-leaving');
      worksCategoryTitle.classList.add('is-entering');
      categoryTitleTimer = window.setTimeout(() => {
        worksCategoryTitle.classList.remove('is-entering');
      }, 220);
    }, 110);
  ...
  const findClosestCard = (trackX = 0) => {
    const focusX = worksViewport.clientWidth / 2;
  ...
  const renderCardDepth = (trackX) => {
    const focusX = worksViewport.clientWidth / 2;
    const distanceRange = Math.max(worksViewport.clientWidth * 0.54, 1);
    const reduceMotion = reducedMotionMedia.matches;

    workCards.forEach((card) => {
      const cardCenter = card.offsetLeft + card.offsetWidth / 2 + trackX;
      const signedDistance = (cardCenter - focusX) / distanceRange;
      const distance = Math.min(1, Math.abs(signedDistance));
      const easedDistance = distance * distance * (3 - 2 * distance);
      const scale = reduceMotion ? 1 : 1 - easedDistance * 0.12;
      const opacity = reduceMotion ? 1 : 1 - easedDistance * 0.55;
      const offsetY = reduceMotion ? 0 : easedDistance * 18;
      const rotateY = reduceMotion ? 0 : Math.max(-1, Math.min(1, signedDistance)) * -2.5;

      card.style.setProperty('--work-track-scale', scale.toFixed(4));
      card.style.setProperty('--work-track-opacity', opacity.toFixed(4));
      card.style.setProperty('--work-track-y', `${offsetY.toFixed(2)}px`);
      card.style.setProperty('--work-track-rotate', `${rotateY.toFixed(2)}deg`);
    });

    setActiveCategory(findClosestCard(trackX)?.dataset.projectCategory);
  };

  const measureWorksTrack = () => {
    window.cancelAnimationFrame(worksFrame);
    window.cancelAnimationFrame(mobileFrame);

    if (!desktopMedia.matches) {
      geometry = null;
      worksRunway.style.removeProperty('--works-runway-height');
      worksTrack.style.removeProperty('--works-track-x');
      workCards.forEach((card) => {
        card.style.removeProperty('--work-track-scale');
        card.style.removeProperty('--work-track-opacity');
        card.style.removeProperty('--work-track-y');
        card.style.removeProperty('--work-track-rotate');
      });
    ...
    const viewportWidth = worksViewport.clientWidth;
    ...
    const runwayHeight = window.innerHeight + travel * 1.08;

    worksRunway.style.setProperty('--works-runway-height', `${runwayHeight}px`);
    geometry = { startX, endX, travel, runwayHeight };
    updateDesktopTrack();
  };

  const updateDesktopTrack = () => {
    window.cancelAnimationFrame(worksFrame);
    worksFrame = window.requestAnimationFrame(() => {
      if (!desktopMedia.matches || !geometry) {
        return;
      }

      const runwayTop = window.scrollY + worksRunway.getBoundingClientRect().top;
      const scrollLength = Math.max(1, geometry.runwayHeight - window.innerHeight);
      const progress = Math.max(0, Math.min(1, (window.scrollY - runwayTop) / scrollLength));
      const trackX = geometry.startX + (geometry.endX - geometry.startX) * progress;

      worksTrack.style.setProperty('--works-track-x', `${trackX.toFixed(2)}px`);
      worksRunway.style.setProperty('--works-progress', progress.toFixed(4));
      renderCardDepth(trackX);
    });
  };

  const updateMobileCategory = () => {
    window.cancelAnimationFrame(mobileFrame);
    mobileFrame = window.requestAnimationFrame(() => {
      ...
      setActiveCategory(closestCard?.dataset.projectCategory);
    });
  };

  window.addEventListener('scroll', updateDesktopTrack, { passive: true });
  worksViewport.addEventListener('scroll', updateMobileCategory, { passive: true });
  window.addEventListener('resize', measureWorksTrack, { passive: true });
  desktopMedia.addEventListener('change', measureWorksTrack);
  ...
  setActiveCategory('ai-product');
  window.requestAnimationFrame(measureWorksTrack);
}
...
if (interestStage) {
  ...
  advanceInterestMode = switchInterest;
...
if (
  ...
  let projectPointerIntent = null;

  document.addEventListener(
    'pointerdown',
    (event) => {
      if (!event.isPrimary || (event.pointerType === 'mouse' && event.button !== 0)) {
        projectPointerIntent = null;
        return;
      }

      const target = event.target instanceof Element ? event.target : null;
      const card = target?.closest('[data-project-gallery]');
      if (!card || !worksViewport?.contains(card)) {
        projectPointerIntent = null;
        return;
      }

      projectPointerIntent = {
        card,
  ...
  document.addEventListener(
    'pointerup',
    (event) => {
      const intent = projectPointerIntent;
      projectPointerIntent = null;
      ...
      window.setTimeout(() => {
        if (!projectViewer.classList.contains('is-open') && document.contains(intent.card)) {
          openProjectViewer(intent.card.dataset.projectGallery);
        }
      }, 0);
    },
    true
...
const initHeroAsciiRipple = () => {
  ...
  const draw = (time = 0, force = false) => {
    ...
    if (!force && time - lastFrameTime < 34) return;
    lastFrameTime = time;
  ...
  const start = () => {
    if (!animationFrame && isVisible && !reducedMotion) {
      animationFrame = requestAnimationFrame(loop);
    }
  };
...
const initAboutBentoScrollReveal = () => {
  if (!aboutSection || !aboutBentoInner || !aboutBentoCards.length) return;
  ...
  aboutBentoCards.forEach((card) => card.classList.add('is-scroll-reveal-ready'));
  aboutBentoInner.classList.add('is-pocket-clipped');

  const setPocketDistances = () => {
    const pocketBottom = aboutBentoInner.offsetTop + aboutBentoInner.offsetHeight;
    aboutBentoCards.forEach((card) => {
      const distanceToPocket = Math.max(80, pocketBottom - card.offsetTop + 24);
      card.style.setProperty('--bento-pocket-distance', `${distanceToPocket}px`);
    });
  };

  const getCardRows = () => {
    const orderedCards = [...aboutBentoCards].sort(
      (first, second) => first.offsetTop - second.offsetTop || first.offsetLeft - second.offsetLeft
    );
    const rows = [];

    orderedCards.forEach((card) => {
      const existingRow = rows.find((row) => Math.abs(row.top - card.offsetTop) < 24);
      if (existingRow) {
        existingRow.cards.push(card);
      } else {
        rows.push({ top: card.offsetTop, cards: [card] });
      }
    });
  ...
  const playReveal = (direction) => {
    ...
    aboutBentoInner.classList.add('is-pocket-clipped');
    setPocketDistances();
    aboutSection.dataset.bentoRevealDirection = direction;
    aboutBentoCards.forEach((card) => {
      card.classList.remove('is-scroll-reveal-visible');
    });

    rows.forEach((row, rowIndex) => {
      row.cards.forEach((card) => {
        card.style.setProperty('--bento-reveal-delay', `${rowIndex * 120}ms`);
      });
    });

    void aboutSection.offsetWidth;
    requestAnimationFrame(() => {
      aboutBentoCards.forEach((card) => card.classList.add('is-scroll-reveal-visible'));
      revealCompleteTimer = window.setTimeout(() => {
        aboutBentoInner.classList.remove('is-pocket-clipped');
      }, rows.length * 120 + 980);
    });
  };
  ...
  const bentoObserver = new IntersectionObserver(
    ([entry]) => {
      if (entry.isIntersecting && !sectionIsActive) {
        sectionIsActive = true;
        playReveal(scrollDirection);
        return;
      }
      ...
      if (!entry.isIntersecting && sectionIsActive) {
        ...
        window.clearTimeout(revealCompleteTimer);
        aboutBentoInner.classList.add('is-pocket-clipped');
        aboutBentoCards.forEach((card) => card.classList.remove('is-scroll-reveal-visible'));
      }
    },
    {
  ...
  setPocketDistances();
  const bentoResizeObserver = new ResizeObserver(setPocketDistances);
  bentoResizeObserver.observe(aboutBentoInner);
  bentoObserver.observe(aboutSection);
};

initAboutBentoScrollReveal();
...
if (
  ...
  let interestDragCycleTimer = null;

  const startInterestDragCycle = (card) => {
    if (card !== interestCard || !advanceInterestMode || interestDragCycleTimer) {
      return;
    }

    interestDragCycleTimer = window.setInterval(advanceInterestMode, 2500);
  };

  const stopInterestDragCycle = () => {
    if (!interestDragCycleTimer) {
      return;
    }

    window.clearInterval(interestDragCycleTimer);
    interestDragCycleTimer = null;
  };
...
const initAboutASCIIHover = () => {
  const targets = [
    ...document.querySelectorAll(
      '.about-profile-copy > p, .about-profile-question > span, .about-profile-question > strong'
    ),
  ];
  const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  const hasFinePointer = window.matchMedia('(hover: hover) and (pointer: fine)').matches;

  if (!targets.length || reduceMotion || !hasFinePointer) {
    return;
  }

  const effectCharacters = '.·:+-';
  const duration = 460;
  const edgeWidth = 1.8;
  const maxRadius = 9;
  ...
  targets.forEach((target) => {
    ...
    target.textContent = '';
    target.classList.add('about-ascii-ripple');
    target.setAttribute('aria-label', originalText);

    const original = document.createElement('span');
    original.className = 'about-ascii-ripple-original';
    original.textContent = originalText;

    const overlay = document.createElement('span');
    overlay.className = 'about-ascii-ripple-overlay';
    overlay.setAttribute('aria-hidden', 'true');
    overlay.textContent = originalText;

    target.append(original, overlay);

    const originalTextNode = original.firstChild;
    const originalCharacters = [...originalText];
    let waves = [];
    ...
    const positionFromPointer = (event) => {
      ...
      if (caret?.offsetNode === originalTextNode) {
        return Math.max(0, Math.min(caret.offset, originalCharacters.length - 1));
      }

      const legacyRange = document.caretRangeFromPoint?.(event.clientX, event.clientY);
      if (legacyRange?.startContainer === originalTextNode) {
        return Math.max(0, Math.min(legacyRange.startOffset, originalCharacters.length - 1));
      }

      const rect = target.getBoundingClientRect();
      const progress = Math.max(0, Math.min(1, (event.clientX - rect.left) / rect.width));
      return Math.round(progress * (originalCharacters.length - 1));
    };

    const scrambledText = (time) =>
      originalCharacters
        .map((character, index) => {
          ...
          waves.forEach((wave) => {
            ...
            if (edgeDistance > 0 && edgeDistance <= edgeWidth) {
              const glyphIndex =
                (distance * 3 + Math.floor(age / 42) + wave.seed) % effectCharacters.length;
              replacement = effectCharacters[glyphIndex];
            }
          });
    ...
    const stop = () => {
      overlay.textContent = originalText;
      target.classList.remove('is-ascii-active');
      animationFrame = null;
    };

    const animate = () => {
      const time = Date.now();
      waves = waves.filter((wave) => time - wave.startedAt < duration);
      if (!waves.length) {
        stop();
        return;
      }

      overlay.textContent = scrambledText(time);
    ...
    const addWave = () => {
      waves.push({
        ...
        startedAt: Date.now(),
        seed: Math.floor(Math.random() * effectCharacters.length),
      });
      waves = waves.slice(-3);
      lastWaveAt = Date.now();
      target.classList.add('is-ascii-active');
    ...
    target.addEventListener('pointerenter', (event) => {
      cursorPosition = positionFromPointer(event);
    ...
    target.addEventListener('pointermove', (event) => {
      cursorPosition = positionFromPointer(event);
      if (
        Math.abs(cursorPosition - previousPosition) < 3 ||
        Date.now() - lastWaveAt < 90
      ) {
        return;
      }
